...
class Attacker(object):
    ...
    def Grad_index(self,words,y,recompute,list_closest_neighbors):
    ...
    def attack(self, count, doc, y, NoAttack,log_f):
        ...
        # check if the value oSet_selectf this doc to be right
        if self.opt.model == 'LSTM':
            doc_var = text_to_var([words], self.src_vocab)
        else:
        ...
        while robust_flag == 1 and n_change <= Budget and time.time() - st <= SECONDS:  # no attack success condition only 0.2 original feature
            ...
            for set_ in Set_c:  # all the combination set
                ...
                feature_n, (Feat_max, cand_max), pred_max = self.WordNeigbor_select(new_words, orig_pred_new,Set_residual,
                                                                            list_closest_neighbors, y, set_,N_REPLACE,recompute)
            ...
            # print("Set_select,arm_chain",Set_select,arm_chain, file=log_f, flush=True)
            if len(arm_chain) !=0:
            ...
            if len(Set_residual) != 0:
                Set_residual = DiffElem(Set_residual, [Set_best[-1][0]])
            else: #
                print(' Set_residual is empty',file=log_f, flush=True)
                break
            Set_c = list(powerset(Set_best))
            # print("Set_best",Set_best,file=log_f, flush=True)
            # print("pred_best", pred_best, file=log_f, flush=True)
            if pred_best >= TAU:
                robust_flag = 0
                break
            Time =time.time() - st
            if Time> SECONDS:
                break
            query_num_each = N_REPLACE * N_NEIGHBOR * math.pow(2,iteration)
...
def main():
    opt = parse_args()
    log_f = open(
        './Logs/OMPGS_L_Budget=%s_N=%s_TAU=%s_N_REPLACE=%s_Time=%s.bak' % (str(Budget),str(N_NEIGHBOR), str(TAU), str(N_REPLACE),str(SECONDS)),
        'w+')

    TITLE = '=== ' + 'OMPGS_N' + ' target prob = ' + str(TAU) + ' changeRange = ' + str(N_REPLACE) + '_Time'+ str(SECONDS)+' ==='
