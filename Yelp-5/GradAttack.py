...
class Attacker(object):
    ...
    def word_paraphrase(self, words, poses, list_neighbors, y):
        ...
        if candidates:
            if self.opt.model == 'LSTM':
                ...
                new_words = candidates[best_candidate_id]
                pred_prob = pred_prob
        else:
    ...
    def attack(self, count, doc, y, NoAttack):
        ...
        while (pred == y or pred_prob < TAU) and n_change <= Budget and time.time() - st < SECONDS \
                and n_change < self.DELTA_W * lword and len(changed_pos) + N_REPLACE < len(
            words):  # no attack success condition only 0.2 original feature
            iteration += 1
            ...
            words, pred_prob, N_CHANGE = self.word_paraphrase(words, indices[:N_REPLACE], list_closest_neighbors,
                                                              y)  # get the new word
            num_searched  = num_searched + list(indices[:N_REPLACE])
            n_change = sum([0 if words_before[i] == words[i] else 1 for i in range(len(words))])
            if n_change > Budget:
                n_change = sum([0 if words_before[i] == best_words[i] else 1 for i in range(len(best_words))])
                break
        ...
        for i in range(len(best_words)):
            if words_before[i] != best_words[i]:
        ...
        for j in range(1,N_REPLACE+1):
            C_value = j * self.nCr(N_REPLACE,j)
            a = a + C_value

        query_num = a * N_NEIGHBOR * iteration
        return best_words, best_score, n_change if best_words else 0, changed_words, Time, iteration, NoAttack,num_searched,query_num


def main():
    opt = parse_args()
    log_f = open(
        './Logs/New_Qilei_L_Budget=%s_Neighbor=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.bak' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)),
        'w+')

    TITLE = '=== ' + 'Qilei' + ' target prob = ' + str(TAU) + ' changeRange = ' + str(N_REPLACE) + ' ==='
    ...
    for count, doc in enumerate(X):
        ...
        changed_doc, flag, num_changed, changed_words, Time, iteration, NoAttack,num_searched,query_num = attacker.attack(count, doc, y[count],
                                                                                                   NoAttack)
        ...
        if v > TAU:
            ...
            Iter = Iter + iteration
            N_Sear = N_Sear+ len(num_searched)
        ...
        if ((count + 1) - NoAttack) != 0 and suc != 0:
            ...
            AverIter = Iter/suc
            AverN_Sear = N_Sear/suc
            AverQuery = Query/suc
            print("SuccessRate = %f, AverTime = %f, AverChanged = %f,AverIter = %f,AverQuery = %f,AverN_Sear = %f,changed_words: %s" % (
                SR, AverTime, AverChanged, AverIter,AverQuery,AverN_Sear,str(changed_words)), file=log_f, flush=True)
