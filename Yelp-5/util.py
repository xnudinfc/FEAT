...
def classify_all(var, model):
    if 'CNN' in str(type(model)):
        ...
        return exp(prob), clazz,torch.exp(pred[0])

def classify_label(var, model,y):
    if 'CNN' in str(type(model)):
        pred,_ = model(var)
        prob, clazz = pred[0].max(dim=0)
        prob_y = pred[0][y]
        return prob.data[0], clazz.data[0],exp(prob_y)
    else:
        pred = model(var)
        prob, clazz = pred[0].max(dim=0)
        prob_y = pred[0][y]
        return exp(prob), clazz,exp(prob_y)
