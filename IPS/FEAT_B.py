        ...
        for i, candi_sample_index in enumerate(poses):
            ...
            if len(candidates) != 1:
                ...
                for index in range(n_batches):  # n_batches
                    batch_diagnosis_codes = torch.LongTensor(candidates[batch_size * index: batch_size * (index + 1)])
...
def main():
    ...
    alpha = opt.alpha
    make_dir('./Logs/')
    make_dir('./Logs/FEAT_B/')
    log_f = open(
        './Logs/FEAT_B/Budget_%s_ALPHA=%s_N_REPLACE=%s_Time=%s.bak' % (
